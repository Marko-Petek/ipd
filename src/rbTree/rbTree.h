/* MP: Taken (and slightly modified) from Zephyr project:
 ...
 *
 * Docs:
 * https://docs.zephyrproject.org/latest/kernel/data_structures/rbtree.html
 *
 * This is an 'intrusive red-black tree' which makes it lightweight.
 *
 * Note: 'CONFIG_MISRA_C' refers to 'MISRA C' which is a security-restricted
 * version of C (e.g., no variable length arrays). We won't be needing this.
 */
 ...
 * usage. The data structure is intrusive, which is to say the 'rbnode'
 * handle is intended to be placed in a separate struct and requires
 * no data pointer to be stored in the node.
 * The color bit is unioned with a pointer (fairly common
...
#include <stdint.h>
#include <alloca.h>
#include <sys/cdefs.h>                                                      // For __containerof() macro.


/** @brief Balanced red/black tree node structure.
 */
typedef struct rbnode {
	struct rbnode *children[2];     // INTERNAL_HIDDEN. Use 'z_rb_child(pParent, leftOrRight)' to access.
} RbNode;
...
typedef struct rbtree
{
	...
	rb_lessthan_t lessthan_fn;                          // Comparison function for nodes in the tree.
	int max_depth;                                      // INTERNAL_HIDDEN.
...
#else
#define _RB_FOREACH_INIT(tree, node) {					                                \
	.stack   = (struct rbnode **) alloca((tree)->max_depth * sizeof(struct rbnode *)),  \
...
#define RB_FOR_EACH_CONTAINER(tree, node, field)		           \
	for (struct _rb_foreach __f = _RB_FOREACH_INIT(tree, node);	   \
			({struct rbnode *n = z_rb_foreach_next(tree, &__f);    \
			 node = n ? __containerof(n, __typeof__(*(node)),       \
					 field) : NULL; }) != NULL;                    \
			 /**/)
